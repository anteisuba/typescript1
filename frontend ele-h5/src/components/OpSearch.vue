...
<template>
    ...
    :style="{ background }">
        <!-- 修正类名 op-search_content -> op-search__content -->
        <div class="op-search__content" :class="shape ? `op-search__content--${shape}` : ``">
            <!-- 修正类名 op-search_field -> op-search__field -->
            <div class="op-cell op-search__field">
                <!-- 修正类名 op-field_left-icon -> op-field__left-icon -->
        ...
        <div v-if="showAction" class="op-search__action">
            <slot name="action">
                <div @click="emits('cancel')">キャンセル</div>
            </slot>
        </div>
    </div>
</template>
...
// 引入 vant 的基础变量
    :root {
        ...
        --van-field-clear-icon-color: #c8c9cc;
        --van-field-right-icon-color: #969799;
    ...
    :root {
        ...
        --op-search-action-padding:var(--van-padding-xs);
